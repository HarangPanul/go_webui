<script lang="ts">
  ...
  // 차이가 뚜렷하게 드러남).
  //
  // RGB 값을 그대로 선형 보간하면 두 끝 색(파랑/빨강)이 중간 지점에서 채도가 빠진
  // 탁한 회색조로 뭉개져, 승률이 애매한 후보가 오히려 가장 나쁜 후보(원색 그대로인
  // 빨강)보다 눈에 덜 띄는 역효과가 난다. 그래서 RGB가 아니라 HSL의 색상(H)만
  // 파랑<->빨강 사이로 보간하고 채도(S)/명도(L)는 항상 고정해서, 중간 지점(주황/
  // 노랑/초록/청록)도 항상 원색 수준으로 선명하게 유지되게 함.
  //
