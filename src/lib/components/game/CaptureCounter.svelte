<script lang="ts">
  // 화면 맨 오른쪽 아래에 떠 있는 포로(따낸 돌) 수 배지. 값은 gameTreeStore.captures를
  ...
  // board-updated 이벤트가 매 수마다 오므로(gameTree.svelte.ts 참고) 최종 결과만
...
<div class="capture-counter" title={t("game.captures")}>
  <span class="entry">
    ...
    {gameTreeStore.captures.black}
  </span>
  <span class="entry">
    <img src={stoneWhiteUrl} alt="white" />
    {gameTreeStore.captures.white}
  </span>
</div>
