...
//
// 게임 트리의 노드마다 결과를 따로 캐싱한다(Map<nodeId, NodeAnalysis>). "최신 결과
// 하나 + 그 결과가 어느 색 기준인지(forColor)"를 따로 들고 있는 방식은 위험하다 -
// 착수 직후 kata-analyze가 새 위치 기준으로 재시작되면 forColor는 곧바로(동기적으로)
// 새 색으로 바뀌는 반면 실제 결과(latest)는 새 스트림의 첫 "info" 줄이 도착할 때까지
// (수백ms~) 옛 위치의 데이터를 그대로 들고 있게 되므로, 그 사이 짧은 순간 옛 위치에서
// 계산된 값이 이미 바뀐 forColor 기준으로 잘못 재해석되어 승률/ownership이 착수할
// 때마다 잠깐 반대로 튀었다가 새 데이터가 오면 되돌아오는(요동치는) 결과를 낳는다.
//
// 그래서 결과와 그 결과가 어느 노드/색 기준인지를 항상 같은 이벤트로 함께 받는다
// (KataAnalyzeEvent - Rust가 kata-analyze 명령을 보내는 바로 그 시점에 노드/색을
// 기록해두므로 둘이 어긋날 일이 없다). 노드 id를 키로 캐싱하므로, 화면에 표시할
// 때도 항상 "같은 노드에 대해 함께 도착한 값" 한 쌍만 보여주게 된다. 부수 효과로
// 두 가지가 함께 해결된다:
